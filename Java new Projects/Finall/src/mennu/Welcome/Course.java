...
import login.login;
import mennu.Welcome.view.crseExtension;
public class Course  extends JFrame{
          ...
          JTextField JT_id,JT_crsename;
           ...
           String s,d, instruction;
            Statement st;
            ResultSet rs;
          JButton save,btn_first,find,btn_next,btn_previous,btn_last,view,btn_update,btn_delete;
          int pos = 0;       
          
          String coursid ,crsename;
         JFrame f3;
...
//get the Connection With Mysql database
    ...
    public static List<crseExtension> BindList(){
        try{
            Connection con = getConnection();
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("select * from course");
            List<crseExtension> list = new ArrayList<crseExtension>();
            while(rs.next()){
                crseExtension u = new crseExtension((rs.getString("coursid")),
                                     rs.getString("crsename")
   ...
   //Showing the Users Info in jtexfields
    public void ShowPosInfo(int index){
        JT_id.setText((BindList().get(index).getCourseid()));
        
        JT_crsename.setText(BindList().get(index).getCrsename());
    ...
    public Course(){
       ...
       f3 = new JFrame("                            WELCOME TO COURSES");
    f3.setSize(700,450);
 ...
 f3.setJMenuBar(jmb);
  ...
  label.setBorder(border);
       label.setBounds(0, 0, 700, 70);
         ...
         heading= new JLabel("<html>welcome to courses panel</html>", SwingConstants.CENTER);
           heading.setForeground(Color.BLACK);
            heading.setBounds(30, 80, 550, 30);
            
            
    
     crseid = new JLabel("Course Id:");
     courseName = new JLabel("CourseName:");
     
    
     
    crseid.setBounds(170, 110, 100, 20);
     courseName.setBounds(170, 140, 100, 20);
     ...
     JT_crsename.setBounds(350, 140, 150, 20);
...
try {
        ...
        String sql="DELETE FROM course WHERE courseid='" + JT_id.getText() + "'";
...
btn_previous.addActionListener(new ActionListener(){
 ...
 private void AddtoRecord()
    {   
         coursid  = JT_id.getText().trim();
     crsename = JT_crsename.getText().trim();
        ...
        st=conn.createStatement();
            System.out.println(coursid  +""+ crsename + " " );
        
             instruction = "Insert into course (coursid ,crsename)" + "VALUES(?,?)";
            PreparedStatement preparedStatement = conn.prepareStatement(instruction);
            preparedStatement.setString(1, coursid );
            preparedStatement.setString(2, crsename);
 ...
 // create a Statement from the connection
    String   sql1  ="update course set crsename = '"+JT_crsename.getText()+"' where coursid  = '" + JT_id.getText() + "'";
