...
public class Programs  extends JFrame{
          ...
          JLabel label,heading,JL_progId,JL_progname;
          JTextField JT_id,JT_progname;
    ...
    public static List<progExtension> BindList(){
        try{
            Connection con = getConnection();
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("select * from programs");
            List<progExtension> list = new ArrayList<progExtension>();
            while(rs.next()){
                progExtension u = new progExtension((rs.getString("progId").trim()),
   ...
   //Showing the Users Info in jtexfields
    public void ShowPosInfo(int index){
        JT_id.setText((BindList().get(index).getProgId()));
        
        JT_progname.setText(BindList().get(index).getProgname());
    ...
    public Programs(){
        f6 = new JFrame("                            WELCOME TO PROGRAMS PANEL");
    f6.setSize(700,450);
...
JMenuItem deptmnt = new JMenuItem("Department");
 ...
 f6.setJMenuBar(jmb);
  ...
  label.setBorder(border);
       label.setBounds(0, 0, 700, 70);
         ...
         heading= new JLabel("<html>welcome to programs panel</html>", SwingConstants.CENTER);
           heading.setForeground(Color.BLACK);
            heading.setBounds(30, 80, 550, 30);
                
    
     JL_progId = new JLabel("Prog Id:");
     JL_progname = new JLabel("Prog Nme:");
    
    JL_progId.setBounds(170, 110, 100, 20);
     JL_progname.setBounds(170, 140, 100, 20);
    


     JT_id = new JTextField(20);
     JT_progname = new JTextField(20);
     
     JT_id.setBounds(350,110,150,20);
     JT_progname.setBounds(350, 140, 150, 20);
 ...
 insertpanel.add(JL_progId );
   insertpanel.add(JL_progname);
   
    insertpanel.add(JT_id);
    insertpanel.add(JT_progname);
...
try {
        ...
        String sql="DELETE FROM programs WHERE progId='" + JT_id.getText().trim() + "'";
...
btn_previous.addActionListener(new ActionListener(){
    ...
    {   
         progId  = JT_id.getText().trim();
     progname = JT_progname.getText().trim();
        ...
        st=conn.createStatement();
            System.out.println(progId  +""+ progname + " " );
        
             instruction = "Insert into programs (progId ,progname)" + "VALUES(?,?)";
 ...
 // create a Statement from the connection
    String   sql1  ="update programs set progname = '"+JT_progname.getText()+"' where progId  = '" + JT_id.getText() + "'";
