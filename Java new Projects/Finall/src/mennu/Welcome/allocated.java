...
import mennu.Welcome.view.allocExtension;
import mennu.Welcome.view.categoryExe;
public class allocated  extends JFrame{

          JLabel label,heading,usrname,JL_itemid,JL_allocid,JL_qty,state,JL_date;
          JTextField JT_allocid,JT_qty,JT_date;
          JComboBox JC_username,Jc_itemid,states;
...
String allocid,username,qty, itemid,datealloc,status;
         JFrame f8;  
 ...
 //get the Connection With Mysql database
    ...
    public static List<allocExtension> BindList(){
        try{
            Connection con = getConnection();
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("select * from allocated");
            List<allocExtension> list = new ArrayList<allocExtension>();
            while(rs.next()){
                allocExtension u = new allocExtension((rs.getString("allocid").trim()),
                                     rs.getString("username"),
                                    rs.getString("itemid"),
                        rs.getString("qty"),
                        rs.getString("datealloc"),
                        rs.getString("status")
    ...
    public void ShowPosInfo(int index){
        JT_allocid.setText((BindList().get(index).getAllocid()));
        JT_qty.setText((BindList().get(index).getQty()));
        JT_date.setText((BindList().get(index).getDatealloc()));
   ...
   public allocated(JFrame parent){
 ...
 JOptionPane.showMessageDialog(null,"ERROR" + e);
    ...
    try{
                  ds= "select * FROM items";
             
      
   rs=st.executeQuery(ds);
     Jc_itemid=new JComboBox();
     Jc_itemid.addItem("");
   while(rs.next())
   {
    
   Jc_itemid.addItem(rs.getString(1));
   }
       ...
       f8 = new JFrame("                            WELCOME TO AllOCATED ITEMS");
    f8.setSize(700,450);
 ...
 f8.setJMenuBar(jmb);
  ...
  label.setBorder(border);
       label.setBounds(0, 0, 700, 70);
         ...
         heading= new JLabel("<html>welcome to allocating items panel</html>", SwingConstants.CENTER);
    ...
    state = new JLabel("Status:");
     ...
     JL_allocid.setBounds(170, 110, 100, 20);
          usrname.setBounds(170, 140, 100, 20);
      JL_itemid.setBounds(170, 170, 100, 20);
...
//insertpanel.add(datePicker);
    ...
    JT_allocid.setBounds(350,110,150,20);
     JC_username.setBounds(350,140,150,20);
    Jc_itemid .setBounds(350, 170, 150, 20);
...
try {
        ...
        String sql="DELETE FROM allocated WHERE allocid='" + JT_allocid.getText() + "'";
...
btn_previous.addActionListener(new ActionListener(){
     ...
     private void AddtoRecord()
    {
        ...
        try
        {
         allocid = JT_allocid.getText().trim();
        username = JC_username.getSelectedItem().toString().trim();
        itemid = Jc_itemid.getSelectedItem().toString();
        qty = JT_qty.getText().trim();
        datealloc=JT_date.getText().trim();
        status=states.getSelectedItem().toString(); 
            
            
        Connection conn = DriverManager.getConnection("jdbc:mysql://localhost/equipment","root","");
        st=conn.createStatement();
            System.out.println(allocid +"" + username + " " +qty + " "+ itemid);
        
             instruction = "Insert into allocated (allocid,username,itemid, qty,datealloc,status)" + "VALUES(?,?,?,?,?,?)";
            PreparedStatement preparedStatement = conn.prepareStatement(instruction);
            preparedStatement.setString(1, allocid);
            preparedStatement.setString(2, username);
            preparedStatement.setString(3, itemid);
            preparedStatement.setString(4, qty);
            preparedStatement.setString(5, datealloc);
...
JOptionPane.showMessageDialog  (null,"" + e);         
    ...
    {   
        ...
        Connection conn = DriverManager.getConnection("jdbc:mysql://localhost/equipment","root","");
        st=conn.createStatement();
            System.out.println(itemid +""+ username + " " + itemid + " " + qty);
 // create a Statement from the connection
    String   sql1  ="update allocated set username = '"+JC_username.getSelectedItem().toString().trim()+"',itemid = '"+Jc_itemid.getSelectedItem().toString().trim()+"',qty = '"+JT_qty.getText().trim()+"',datealloc = '"+JT_date.getText().trim()+"',status = '"+states.getSelectedItem().toString().trim()+"' where allocid = '" + JT_allocid.getText().trim() + "'";
