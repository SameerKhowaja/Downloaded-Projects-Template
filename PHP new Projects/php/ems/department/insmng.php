  ...
  $_GLOBALS['message']="Please, You are accessing unauthorized page.Click here to <a href=\"../index.php\">Re-LogIn</a>";
...
} else if (isset($_REQUEST['insmng'])) {
    /*     * ************************ Step 2 - Case 2 ************************ */
    //redirect to import staff profile

    header('Location: importins.php');
    ...
    foreach ($_REQUEST as $variable) {
        ...
        if (is_numeric($variable)) { //it is because, some session values are also passed with request
            ...
            if (!@executeQuery("delete from instructor where insid=$variable")){
    ...
    if (!isset($_GLOBALS['message']) && $hasvar == true)
        $_GLOBALS['message'] = "Selected Instructor/s are successfully Deleted";
    else if (!$hasvar) {
        $_GLOBALS['message'] = "First Select the Instructor to be Deleted.";
    }
} else if (isset($_REQUEST['savem'])) {
    /*     * ************************ Step 2 - Case 4 ************************ */
    //updating the modified values
     if (empty($_REQUEST['idno']) || empty($_REQUEST['fullname'])|| empty($_REQUEST['newpass']) || empty($_REQUEST['repass'])) {
    	...
    	$u=htmlspecialchars($_REQUEST['idno'], ENT_QUOTES);
    	$fullname=htmlspecialchars($_REQUEST['fullname'], ENT_QUOTES);
    	$sex=htmlspecialchars($_REQUEST['sex'], ENT_QUOTES);
    	$nation=htmlspecialchars($_REQUEST['nation'], ENT_QUOTES);
    	$rank=htmlspecialchars($_REQUEST['rank'], ENT_QUOTES);
    	$phoneno=htmlspecialchars($_REQUEST['phoneno'], ENT_QUOTES);
    	$email=htmlspecialchars($_REQUEST['email'], ENT_QUOTES);
    	
        $query = "update instructor,user set instructor.idno='" . $idno."', instructor.fullname='".$fullname. "',instructor.sex='" .$sex . "',instructor.nationality='" .$nation . "',instructor.academicrank='" .$rank . "',instructor.phoneno='" . $phoneno. "',instructor.email='" .$email. "',user.password='" . $newpass . "' where instructor.insid='" . $_REQUEST['instructor'] . "' and user.uid='".$_REQUEST['user']."';";
        if (!@executeQuery($query))
            $_GLOBALS['message'] = mysql_error();
        else
            $_GLOBALS['message'] = "Instructor Information is Successfully Updated.";
    }
    closedb();
}
...
?>
        <div id="container">
            ...
            <form name="insmng" action="insmng.php" method="post">
                <div class="menubar">
...
?>
                       <li><input type="submit" value="LogOut" name="logout" class="subbtn" title="Log Out"/></li>
                            <li><input type="submit" value="DashBoard" name="dashboard" class="subbtn" title="Dash Board"/></li>
                             <li><input type="submit" value="Student" name="stdmng" class="subbtn" title="Student Management"/></li>
...
?>
                                ...
                                <li><input type="submit" value="Save" name="savea" class="subbtn" onclick="validateform('insmng')" title="Save the Changes"/></li>

                        <?php
                    } else if (isset($_REQUEST['edit'])) { //navigation for Edit option
                        ?>
                        <li><input type="submit" value="Cancel" name="cancel" class="subbtn" title="Cancel"/></li>
                        <li><input type="submit" value="Save" name="savem" class="subbtn" onclick="validateform('insmng')" title="Save the changes"/></li>
                        ...
                        <li><input type="submit" value="Delete" name="delete" class="subbtn" title="Delete" onclick ="return confirm('Are you sure to delete this record?')"/></li>
                         <li><input type="submit" value="Add" name="insmng" class="subbtn" title="Add New Instructor/s"/></li>
...
<?php
if (isset($_SESSION['username'])) {
    echo "<div class=\"pmsg\" style=\"text-align:center;\">Instructor Management </div>";
    if (isset($_REQUEST['add'])) {
        /*         * ************************ Step 3 - Case 1 ************************ */
...
<?php
                        ...
                        $result = executeQuery("select ins.insid,ins.idno,ins.fullname,ins.sex,ins.nationality,ins.academicrank,ins.phoneno,ins.email,u.uid,u.password  from instructor as ins, user as u where ins.idno='" . htmlspecialchars($_REQUEST['edit'],ENT_QUOTES) . "';");
...
?>
                    ...
                    <tr>
                        ...
                        <tr>
                            <td>Nationality</td>
                            <td><input type="text" name="nation" value="<?php echo htmlspecialchars($r['nationality'], ENT_QUOTES); ?>" size="25" onkeyup="isalpha(this)"  onblur="if(this.value==''){alert('Nationality field is Empty');this.focus();this.value='';}"/></td>

                        </tr>
                         <tr>
                            <td>Phone No:</td>
                            <td><input type="text" name="phoneno" value="<?php echo htmlspecialchars($r['phoneno'], ENT_QUOTES); ?>" size="25" onkeyup="isnum(this)"  onblur="if(this.value==''){alert('Phone No field is Empty');this.focus();this.value='';}"/></td>

                        </tr>
                         <tr>
                            <td>Academic Rank:</td>
                            <td><input type="text" name="rank" value="<?php echo htmlspecialchars($r['academicrank'], ENT_QUOTES); ?>" size="25" onkeyup="isnum(this)"  onblur="if(this.value==''){alert('Phone No field is Empty');this.focus();this.value='';}"/></td>

                        </tr>
                         <tr>
                            <td>E-Mail</td>
                            <td><input type="email" name="email" value="<?php echo htmlspecialchars($r['email'], ENT_QUOTES); ?>" size="25"   onblur="if(this.value==''){alert('Email field is Empty');this.focus();this.value='';}"/></td>
                        ...
                        <tr>
                            ...
                            <td><input type="hidden" name="instructor" value="<?php echo htmlspecialchars_decode($r['insid'], ENT_QUOTES); ?>"/>
...
<?php
                        ...
                        $result = executeQuery("select ins.insid,ins.idno,ins.fullname,ins.sex,ins.nationality,ins.academicrank,ins.phoneno,ins.email,u.password  from instructor as ins, user as u where ins.idno=u.username and ins.uid='".$_SESSION['uid']."';");
                      
                        if (mysql_num_rows($result) == 0) {
                            echo "<h3 style=\"color:#0000cc;text-align:center;\">No Instructor/s Profile Yet..!</h3>";
                        } else {
                            $i = 0;
?>
                    ...
                    <?php
                            while ($r = mysql_fetch_array($result)) {
                                ...
                                if ($i % 2 == 0)
                                    ...
                                    echo "<tr>";
                                echo "<td style=\"text-align:center;\"><input type=\"checkbox\" name=\"d$i\" value=\"" . $r['insid'] . "\" /></td><td>$i</td><td>" . htmlspecialchars_decode($r['idno'],ENT_QUOTES)
                                . "</td><td>" . htmlspecialchars_decode($r['fullname'],ENT_QUOTES) . "</td><td>" . htmlspecialchars_decode($r['sex'],ENT_QUOTES) . "</td>"
                                . "</td><td>" . htmlspecialchars_decode($r['nationality'],ENT_QUOTES) . "</td><td>" . htmlspecialchars_decode($r['academicrank'],ENT_QUOTES) . "</td>"
                                . "</td><td>" . htmlspecialchars_decode($r['phoneno'],ENT_QUOTES) . "</td><td>" . htmlspecialchars_decode($r['email'],ENT_QUOTES) . "</td>"
                                . "</td><td>" . htmlspecialchars_decode($r['password'],ENT_QUOTES) . "</td>"
                                . "<td class=\"tddata\"><a title=\"Edit " . htmlspecialchars_decode($r['idno'],ENT_QUOTES) . "\"href=\"insmng.php?edit=" . htmlspecialchars_decode($r['idno'],ENT_QUOTES) . "\"><img src=\"../images/edit.png\" height=\"30\" width=\"40\" alt=\"Edit\" /></a></td></tr>";
                            }
                    ?>
					</tbody>
				<tfoot class="nav">
